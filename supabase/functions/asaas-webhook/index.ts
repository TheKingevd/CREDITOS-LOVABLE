...
Deno.serve(async (request) => {
  ...
  if (!supabaseUrl || !serviceRoleKey || !asaasKey) return json({ error: "Integração segura não configurada" }, 500);
  ...
  // O payload recebido nunca é a fonte final da verdade financeira. Consulte o
  // recurso no Asaas usando a credencial secreta e compare o valor da cobrança.
  ...
  if (!Number.isFinite(remoteValueCents) || remoteValueCents !== sale.amount_cents) {
    await adminClient.from("asaas_webhook_events").update({ processed_at: new Date().toISOString() }).eq("id", eventId);
    return json({ received: true, ignored: true });
  }

  const paymentStatus = String(remotePayment.status || body.payment?.status || "");
  let internalStatus = paymentStatus;
  let saleStatus: string | null = null;
  switch (event) {
    case "PAYMENT_RECEIVED": internalStatus = "RECEIVED"; saleStatus = "pago"; break;
    case "PAYMENT_CONFIRMED": internalStatus = "CONFIRMED"; break;
    case "PAYMENT_OVERDUE": internalStatus = "OVERDUE"; saleStatus = "vencido"; break;
    case "PAYMENT_DELETED": internalStatus = "DELETED"; saleStatus = "cancelado"; break;
    case "PAYMENT_REFUNDED": internalStatus = "REFUNDED"; saleStatus = "estornado"; break;
    default: internalStatus = paymentStatus || event;
  }

  // Nunca rebaixe uma venda já paga por um evento atrasado/fora de ordem.
  if (sale.status === "pago" && saleStatus && saleStatus !== "estornado") saleStatus = null;

  const now = new Date().toISOString();
  const paymentUpdate: Record<string, unknown> = { status: internalStatus, updated_at: now };
  if (event === "PAYMENT_RECEIVED") paymentUpdate.paid_at = now;
  const { error: paymentError } = await adminClient.from("asaas_payments").update(paymentUpdate).eq("id", integration.id);
  if (paymentError) return json({ error: "Falha ao atualizar pagamento" }, 500);

  if (saleStatus) {
    const { error: saleError } = await adminClient.from("sales").update({ status: saleStatus }).eq("id", integration.sale_id);
