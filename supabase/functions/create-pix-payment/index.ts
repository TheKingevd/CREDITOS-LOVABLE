...
Deno.serve(async (request) => {
  ...
  if (!supabaseUrl || !anonKey || !serviceRoleKey || !asaasKey) {
    return json({ error: "Integração Asaas não configurada. Defina ASAAS_API_KEY e as variáveis do Supabase." }, 500);
  }
  ...
  const { data: existing } = await adminClient
    .from("asaas_payments")
    .select("asaas_payment_id, pix_payload, pix_encoded_image, pix_expiration_date, status")
    .eq("sale_id", sale.id)
    .maybeSingle();

  if (existing?.pix_payload && existing.pix_encoded_image) {
    return json({
      paymentId: existing.asaas_payment_id,
      payload: existing.pix_payload,
      encodedImage: existing.pix_encoded_image,
  ...
  if (!sale.customer_document) {
    return json({ error: "Informe o CPF/CNPJ do cliente para gerar o Pix Asaas." }, 400);
  }
  ...
  const customerLookup = await fetch(customerLookupUrl, {
  ...
  if (!customer) {
    const customerResponse = await fetch(`${asaasBaseUrl}/customers`, {
      ...
      body: JSON.stringify({
        ...
        cpfCnpj: String(sale.customer_document).replace(/\D/g, ""),
        email: sale.customer_email || undefined,
        mobilePhone: sale.customer_phone || undefined,
        externalReference: sale.id,
      }),
    });
    if (!customerResponse.ok) {
      const errorBody = await customerResponse.text();
      return json({ error: `Não foi possível cadastrar o cliente no Asaas: ${errorBody}` }, 502);
    }
  ...
  const paymentResponse = await fetch(`${asaasBaseUrl}/payments`, {
    ...
    body: JSON.stringify({
      ...
      billingType: "PIX",
      value: Number(sale.amount_cents) / 100,
      dueDate,
      description: `${sale.plan_name} — ${sale.customer_name}`.slice(0, 500),
      externalReference: sale.id,
    }),
  });
  ...
  if (!paymentResponse.ok) {
    ...
    return json({ error: `Não foi possível criar a cobrança Pix: ${errorBody}` }, 502);
  }

  const payment = await paymentResponse.json();
  const qrResponse = await fetch(`${asaasBaseUrl}/payments/${payment.id}/pixQrCode`, {
    headers: { accept: "application/json", access_token: asaasKey },
  });

  if (!qrResponse.ok) {
    const errorBody = await qrResponse.text();
    return json({ error: `Cobrança criada, mas não foi possível obter o QR Code: ${errorBody}` }, 502);
  }

  const qr = await qrResponse.json();
  const { error: saveError } = await adminClient.from("asaas_payments").upsert({
    sale_id: sale.id,
    ...
    pix_expiration_date: qr.expirationDate,
    updated_at: new Date().toISOString(),
  }, { onConflict: "sale_id" });

  if (saveError) return json({ error: "Cobrança criada, mas não foi possível registrar a integração local." }, 500);
