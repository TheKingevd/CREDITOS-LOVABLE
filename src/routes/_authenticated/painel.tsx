...
const saleSchema = z.object({
  ...
  customer_document: z.string().trim().refine((value) => [11, 14].includes(value.replace(/\D/g, "").length), "Informe um CPF ou CNPJ válido"),
...
function PainelPage() {
  ...
  if (isLoading) {
    return <AppShell title="Painel da revenda"><p className="text-muted-foreground">Carregando…</p></AppShell>;
  }

  if (!reseller) {
    return <AppShell title="Painel da revenda"><NoReseller isAdmin={!!account?.isAdmin} /></AppShell>;
  }
    ...
    <AppShell title={`Revenda ${reseller.name}`}>
      ...
      {!reseller.active && (
        <div className="panel mt-6 border-destructive/40 p-5">
          ...
          <p className="mt-1 text-sm text-muted-foreground">O registro de vendas está bloqueado. Fale com o administrador para reativar seu acesso.</p>
        </div>
      )}
...
function NoReseller({ isAdmin }: { isAdmin: boolean }) {
  ...
  const claim = useMutation({
    ...
    onSuccess: (ok) => {
      ...
      } else toast.error("Já existe um administrador nesta plataforma.");
    },
  ...
  return (
    <div className="panel max-w-xl p-6">
      ...
      <p className="mt-2 text-sm text-muted-foreground">Peça ao administrador para cadastrar sua revenda com este mesmo e-mail. Assim que o cadastro existir, o painel é liberado automaticamente.</p>
      {!isAdmin && <Button className="mt-5" variant="outline" onClick={() => claim.mutate()}>Sou o dono da plataforma</Button>}
    </div>
  );
}
  ...
  const sell = useMutation({
    mutationFn: async () => {
      ...
      const { data: sale, error } = await supabase.from("sales").insert({
        ...
        customer_document: parsed.data.customer_document.replace(/\D/g, ""),
        credits: Number(credits) || 0,
        amount_cents: amountCents,
        commission_cents: commissionCents,
        payment_method: method,
        status: method === "pix" ? "aguardando_pagamento" : "pago",
    ...
    onSuccess: (sale) => {
      ...
      setCredits("0");
      if (method === "pix") {
        setPixSaleId(sale.id);
        toast.success("Venda criada. Gere o Pix para o cliente.");
      } else {
  ...
  return (
    <>
      <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
        <div className="panel space-y-4 p-6">
          ...
          <div className="space-y-2">
            ...
            <Select value={planId} onValueChange={pickPlan}>
              ...
              <SelectContent>{(plans ?? []).map((p) => <SelectItem key={p.id} value={p.id}>{p.name} — {p.credits} créditos — {brl(effectivePriceCents(p))}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2"><Label>CPF/CNPJ</Label><Input value={customerDocument} onChange={(e) => setCustomerDocument(e.target.value)} placeholder="Necessário para o Pix Asaas" /></div>
          ...
          <Button className="w-full" disabled={!active || !selected || sell.isPending} onClick={() => sell.mutate()}>
            {sell.isPending ? "Processando…" : method === "pix" ? "Criar venda e gerar Pix" : active ? "Registrar venda" : "Revenda inativa"}
          </Button>
        </div>
        ...
        <div className="panel h-fit space-y-3 p-6">
          ...
          <p className="pt-2 text-xs text-muted-foreground">
            {method === "pix" ? "No Pix, os créditos só são descontados após o Asaas confirmar o recebimento." : "Os créditos são descontados do saldo da revenda assim que a venda é registrada."}
          </p>
        </div>
      </div>
      ...
      <PixPaymentDialog
        open={!!pixSaleId}
        saleId={pixSaleId}
        onOpenChange={(open) => { if (!open) setPixSaleId(null); }}
        onPaid={() => queryClient.invalidateQueries()}
      />
    </>
  );
}
...
function Row({ label, value }: { label: string; value: string }) {
  return <div className="flex items-center justify-between border-b border-border/60 pb-2 text-sm"><span className="text-muted-foreground">{label}</span><span className="font-medium">{value}</span></div>;
}
...
function MySales({ resellerId }: { resellerId: string }) {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("sales").select("*").eq("reseller_id", resellerId).order("created_at", { ascending: false });
      if (error) throw error;
      return data;
    },
  });
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-3"><Stat label="Vendas" value={String(data?.length ?? 0)} /><Stat label="Faturamento" value={brl(total)} /><Stat label="Comissões" value={brl(commission)} /></div>
      <div className="panel divide-y divide-border/60">
        {(data ?? []).map((s) => (
          <div key={s.id} className="flex flex-wrap items-center justify-between gap-2 p-4 text-sm">
            <div>
              <p className="font-medium">{s.customer_name} · {s.plan_name}</p>
              <p className="text-muted-foreground">{formatDate(s.created_at)} · {s.credits} créditos · {s.payment_method}</p>
            </div>
            <div className="text-right"><p className="font-semibold">{brl(s.amount_cents)}</p><p className="text-xs text-muted-foreground">{s.status} · comissão {brl(s.commission_cents)}</p></div>
          </div>
        ))}
...
function MyCredits({ resellerId }: { resellerId: string }) {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("credit_movements").select("*").eq("reseller_id", resellerId).order("created_at", { ascending: false });
      if (error) throw error;
      return data;
    },
  });
    ...
    <div className="panel divide-y divide-border/60">
      {(data ?? []).map((m) => (
        <div key={m.id} className="flex items-center justify-between gap-3 p-4 text-sm">
          <div><p className="font-medium capitalize">{m.reason}</p><p className="text-muted-foreground">{formatDate(m.created_at)} {m.note ? `· ${m.note}` : ""}</p></div>
          <Badge variant={m.delta >= 0 ? "default" : "secondary"}>{m.delta >= 0 ? "+" : ""}{m.delta}</Badge>
        </div>
      ))}
      {!data?.length && <p className="p-4 text-sm text-muted-foreground">Nenhum movimento de crédito.</p>}
    </div>
  );
}
