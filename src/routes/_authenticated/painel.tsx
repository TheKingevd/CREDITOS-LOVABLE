...
const saleSchema = z.object({
  ...
  customer_document: z.string().trim().max(30),
...
function Pdv({ resellerId, commission, active }: { resellerId: string; commission: number; active: boolean }) {
  ...
  const sell = useMutation({
    mutationFn: async () => {
      const parsed = saleSchema.safeParse({ customer_name: customerName, customer_email: customerEmail, customer_phone: customerPhone, customer_document: customerDocument, note });
...
function MySales({ resellerId }: { resellerId: string }) {
  const { data } = useQuery({ queryKey: ["sales", resellerId], queryFn: async () => { const { data, error } = await supabase.from("sales").select("*").eq("reseller_id", resellerId).order("created_at", { ascending: false }); if (error) throw error; return data; } });
  const total = (data ?? []).reduce((acc, s) => acc + s.amount_cents, 0); const commission = (data ?? []).reduce((acc, s) => acc + s.commission_cents, 0);
  return <div className="space-y-4"><div className="grid gap-4 sm:grid-cols-3"><Stat label="Vendas" value={String(data?.length ?? 0)} /><Stat label="Faturamento" value={brl(total)} /><Stat label="Comissões" value={brl(commission)} /></div><div className="panel divide-y divide-border/60">{(data ?? []).map((s) => <div key={s.id} className="flex flex-wrap items-center justify-between gap-2 p-4 text-sm"><div><p className="font-medium">{s.customer_name} · {s.plan_name}</p><p className="text-muted-foreground">{formatDate(s.created_at)} · {s.credits} créditos · {s.payment_method}</p></div><div className="text-right"><p className="font-semibold">{brl(s.amount_cents)}</p><p className="text-xs text-muted-foreground">{s.status} · comissão {brl(s.commission_cents)}</p></div></div>)}{!data?.length && <p className="p-4 text-sm text-muted-foreground">Nenhuma venda ainda.</p>}</div></div>;
}

function MyCredits({ resellerId }: { resellerId: string }) {
  const { data } = useQuery({ queryKey: ["movements", resellerId], queryFn: async () => { const { data, error } = await supabase.from("credit_movements").select("*").eq("reseller_id", resellerId).order("created_at", { ascending: false }); if (error) throw error; return data; } });
  return <div className="panel divide-y divide-border/60">{(data ?? []).map((m) => <div key={m.id} className="flex items-center justify-between gap-3 p-4 text-sm"><div><p className="font-medium capitalize">{m.reason}</p><p className="text-muted-foreground">{formatDate(m.created_at)} {m.note ? `· ${m.note}` : ""}</p></div><Badge variant={m.delta >= 0 ? "default" : "secondary"}>{m.delta >= 0 ? "+" : ""}{m.delta}</Badge></div>)}{!data?.length && <p className="p-4 text-sm text-muted-foreground">Nenhum movimento de crédito.</p>}</div>;
}
